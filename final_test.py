#This project requires installation of tensorflow, dlib, opencv
#google credentials has to be done before implementing the voice module.
#a python class "faceBlendCommon.py" has to be included in the working directory for facial landmarking.
...
def classifyhand(file_name):
    ...
    return text
########google voice recognition module process method #############
flag =1
...
def process_event(event):
    ...
    if event.type == EventType.ON_RECOGNIZING_SPEECH_FINISHED:
        ...
        r=requests.post("http://IP:Port", data ={'direction': speech_text}) ### here mention the ip address of the raspberrypi
...
input_layer = "input"
output_layer = "final_result"
##google voice recognition module#####
...
################ finger module flag ==1 / finger gesture recognition module ################
	if cv2.waitKey(1) & 0xFF == ord('s') or flag == 1:
		...
		if len(rects) != 0:
			...
			#resized = cv2.resize(crphand, dim, interpolation = cv2.INTER_AREA)
		        resized = crphand
		        opdir='/home/bharavi/finger/op_imgs/'						
